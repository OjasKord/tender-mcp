...
- removed: notifyGateHit() -- raw free-tier/bundle gate hits no longer send an email (still increment counters, still return 402). Email now fires only on a trial-extension request or a payment event (Stripe checkout, or an x402 settlement)
- added: Redis-independent in-process circuit breaker (20 emails/hour, embedded in sendEmail() itself) covering every remaining email path -- trial-extension notify/confirm/follow-up, paid API key delivery, and the new x402 settlement alert -- so a Redis outage can't fail-open into an email flood (Lesson 209 pattern)
- added: distinct, louder alert on a real x402 settlement success -- subject "[x402 SETTLEMENT] Tender — real payment received" -- fired fire-and-forget immediately after redisIncr(x402_calls), never blocking or affecting the already-settled, already-delivered response. This is the fleet's key signal for the first real external agent-native payment.

## [1.3.4] - 2026-07-22
- fix: 1.3.3's mainnet arm attempt crashed at Railway healthcheck — ERR_REQUIRE_ESM. @coinbase/x402 pulls in @coinbase/cdp-sdk, whose CJS build require()s jose@6 (ESM-only, no CJS export); Node 18 (Railway's runtime) has no require(esm) support. Fixed by loading @coinbase/x402 via dynamic import() instead of require(), scoped to the X402_NETWORK=base branch only — walks the package's "import" export condition end-to-end so jose loads as ESM throughout and the incompatibility never triggers. base-sepolia path stays fully synchronous, untouched.
- fix: jose@6's WebCrypto build also needs a global `crypto`, which Node 18 doesn't expose by default. Added a one-line polyfill (`globalThis.crypto = globalThis.crypto || crypto.webcrypto`) at the top of the file — a no-op on any Node version that already has it natively (20+).
- fix: dynamic import() is async, so there's a real window after the process starts accepting connections where x402Server exists but initialize() hasn't resolved yet. Added an x402Ready flag, set only on genuine initialize() success (both networks) — a payment attempt arriving before it's true now fails closed with a retryable 402 instead of silently falling through to free-tier accounting.
- Verified this time on the actual Node 18.20.8 binary (not a newer local Node, which masks require(esm)-dependent bugs): reproduced the original crash, confirmed the fix boots with a real facilitator round-trip (gets a legitimate CDP 401 with a non-registered test key, proving the whole pipeline — module loading, JWT signing, network call — works end-to-end), confirmed the fail-loud missing-key guard still exits 1, confirmed dormant and base-sepolia paths are unaffected, and confirmed the not-ready race fails closed via an artificially widened window.
